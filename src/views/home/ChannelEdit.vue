<template>
  <div>
    ...
    <div class="more-channel-box">
      ...
      <!-- 更多频道列表 -->
      <van-row type="flex">
        <van-col span="6" v-for="obj in moreList" :key="obj.id" @click="addChannelFn(obj)">
...
export default {
  ...
  methods: {
    ...
    deleteFn (obj) {
      if (this.isEdit === true) {
        if (obj.id !== 0) {
          this.$emit('deleteChannelEV', obj)
        }
      } else { // 非编辑下进入频道
        this.closeFn()
        this.$emit('input', obj.id) //  v-model会在原地留下一个input事件，调用input事件
        // channelId控制切换频道 用v-model绑定channelId
      }
    },
    closeFn () {
      this.$emit('closeEV')
